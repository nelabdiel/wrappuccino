...
class ModelLoader:
    ...
    def _load_sklearn_model(self):
        ...
        try:
            ...
            logger.info(
                f"Scikit-learn model loaded successfully from {self.config.model_path}"
            )
        except Exception as e:
            ...
            try:
                ...
                logger.info(
                    f"Scikit-learn model loaded successfully with joblib from {self.config.model_path}"
                )
            except ImportError:
                logger.error("joblib not available for model loading")
                raise ValueError(
                    f"Failed to load model - joblib required: {str(e)}")
            except Exception as joblib_error:
                logger.error(
                    f"Failed to load scikit-learn model with joblib: {str(joblib_error)}"
                )
                raise ValueError(
                    f"Model loading failed - incompatible format: {str(joblib_error)}"
                )
    ...
    def _load_pytorch_model(self):
        ...
        try:
            ...
            if arch_file.exists():
                ...
                self.arch_module = importlib.util.module_from_spec(spec)
                spec.loader.exec_module(self.arch_module)
                
                if not hasattr(self.arch_module, 'create_model'):
                    raise ValueError(
                        "model_architecture.py must define a 'create_model()' function"
                    )
                
                # Create model instance
                self.model = self.arch_module.create_model()
                ...
                if isinstance(state_dict, dict):
                    ...
                    logger.info("Loaded PyTorch state dict into model architecture")
                else:
                    # If it's a complete model, use it directly
                    self.model = state_dict
                    logger.info("Loaded complete PyTorch model")
            else:
                # Load complete model (no architecture script needed)
                self.model = torch.load(self.config.model_path, map_location=device)
                
                if isinstance(self.model, dict):
                    raise ValueError(
                        "PyTorch state dict detected but no model_architecture.py found. "
                        "Complex models like BERT require a model_architecture.py file."
                    )
                logger.info("Loaded complete PyTorch model")
    ...
    def _load_onnx_model(self):
        ...
        try:
            ...
            logger.info(
                f"ONNX model loaded successfully from {self.config.model_path}"
            )
            logger.info(
            ...
            raise ValueError(
                "onnxruntime not installed - required for .onnx models")
        except Exception as e:
    ...
    def _load_vectorizer(self):
        ...
        try:
            ...
            logger.info(
                f"Vectorizer loaded successfully from {self.config.vectorizer_path}"
            )
        except Exception as e:
            logger.warning(
                f"Standard pickle loading failed for vectorizer: {str(e)}")
            # Try loading with joblib for better scikit-learn compatibility
            try:
                import joblib
                self.vectorizer = joblib.load(self.config.vectorizer_path)
                logger.info(
                    f"Vectorizer loaded successfully with joblib from {self.config.vectorizer_path}"
                )
            except Exception as joblib_error:
                logger.error(
                    f"Failed to load vectorizer with joblib: {str(joblib_error)}"
                )
                raise ValueError(
                    f"Vectorizer loading failed: {str(joblib_error)}")
    ...
    def _load_preprocessing(self):
        ...
        try:
            ...
            if hasattr(module, 'custom_preprocess'):
                ...
                logger.info(
                    f"Preprocessing function loaded from {self.config.preprocessing_path}"
                )
            else:
                logger.warning(
                    f"No 'custom_preprocess' function found in {self.config.preprocessing_path}"
                )
    ...
    def _load_label_encoder(self):
        ...
        try:
            ...
            logger.info(
                f"Label encoder loaded successfully from {self.config.label_encoder_path}"
            )
        except Exception as e:
            logger.warning(
                f"Standard pickle loading failed for label encoder: {str(e)}")
            ...
            try:
                ...
                logger.info(
                    f"Label encoder loaded successfully with joblib from {self.config.label_encoder_path}"
                )
            except Exception as joblib_error:
                logger.error(
                    f"Failed to load label encoder with joblib: {str(joblib_error)}"
                )
                raise ValueError(
                    f"Label encoder loading failed: {str(joblib_error)}")
    ...
    def predict_text(self, text: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.preprocessing_func:
                ...
                logger.debug(
                    f"Preprocessing applied: '{text}' -> '{processed_text}'")
            ...
            if self.label_encoder and hasattr(self.label_encoder,
                                              'inverse_transform'):
                try:
        ...
        try:
            ...
            if hasattr(self.model, 'predict_proba'):
                try:
                    ...
                    logger.debug(
                        f"Could not get prediction probability: {str(e)}")
